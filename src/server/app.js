  ...
  constructor(c) {
    ...
    this.app.use((req, res, next) => {
      if (this.c.domain === "*") {
        ...
        req.headers.host !== this.c.domain.toLowerCase() &&
        !this.c.domain.includes("*")
      ) {
        res.statusCode = 401;
    ...
    this.app.use((req, res, next) => {
      ...
      if (exists === undefined) {
        // if a ban was not found, then it will move on
        next();
      } else {
        res.statusCode = 401;
    ...
    this.app.use((req, res, next) => {
      if (req.method === "GET") {
        ...
        if (exists === undefined) {
          ...
          const trafficExists = this.db
            .get("trafficTotal")
            .find({ month: trafficPeriod })
            .value(); // traffic exists for this month and year
