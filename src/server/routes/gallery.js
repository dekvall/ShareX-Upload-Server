...
async function post(req, res) {
  ...
  if (!this.c.admin.key.includes(req.body.password)) {
    ...
    return this.log.warning(
      `Unauthorized User | Gallery Access | ${userIP} | ${req.body.password}`
    );
  }
  this.log.warning(
    `IP Address: ${userIP} successfully accessed gallery with key ${req.body.password}`
  );
  fs.readdir(`${__dirname}/../uploads`, (err, files) => {
    ...
    files = files.map((fileName) => {
      return {
        name: fileName,
        time: fs
          .statSync(`${__dirname}/../uploads/${fileName}`)
          .mtime.getTime(),
      };
    });
    files.sort((a, b) => {
