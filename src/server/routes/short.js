...
async function post(req, res) {
  ...
  stream.once("open", () => {
    stream.write(
      `<meta http-equiv="refresh" content="0;URL='${req.body.URL}'" />`
    );
    stream.end();
    this.log.verbose(
      `New Short URL: ${protocol}://${req.headers.host}/${fileName} | IP: ${userIP}`
    );
    res.redirect(
      `/short?success=${protocol}://${req.headers.host}/${fileName}`
    );
    this.db
      .get("files")
